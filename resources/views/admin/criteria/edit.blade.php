...
@section('content')
    @php
        $oldSkala = old('skala', $criteria->skala ?? '');
        $oldPreferenceFunction = old('preference_function', $criteria->preference_function ?? '');
        $oldRows = old(
            'ordinal',
            $criteria->ordinals
                ->map(
    ...
    <div class="px-4 py-6 mx-auto">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-semibold text-[var(--flora-moss)] tracking-tight">
                    Edit Kriteria "{{ $criteria->nama_criteria }}"
                </h1>
        ...
        <form action="{{ route('criteria.update', $criteria->id) }}" method="POST" x-data="criteriaForm()">
            @csrf
            @method('PUT')
                ...
                <div class="px-6 py-6 space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" id="nama_kriteria" name="nama_kriteria"
                                value="{{ old('nama_kriteria', $criteria->nama_criteria) }}"
                        ...
                        <div>
                            ...
                            <select id="tipe" name="tipe"
                                ...
                                <option value="benefit" {{ old('tipe', $criteria->tipe) == 'benefit' ? 'selected' : '' }}>
                                    Benefit — semakin besar semakin baik
                                </option>
                                <option value="cost"    {{ old('tipe', $criteria->tipe) == 'cost'    ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" id="satuan" name="satuan" value="{{ old('satuan', $criteria->satuan) }}"
                        ...
                        <div>
                            ...
                            <select id="skala" name="skala" x-model="skala"
                                ...
                                <option value="numerik" {{ old('skala', $criteria->skala) == 'numerik' ? 'selected' : '' }}>
                                    Numerik — input angka langsung
                                </option>
                                <option value="ordinal" {{ old('skala', $criteria->skala) == 'ordinal' ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="number" id="bobot" name="bobot" value="{{ old('bobot', $criteria->bobot) }}" min="0"
                    ...
                    <div>
                        ...
                        <select id="preference_function" name="preference_function" x-model="preferenceFunction"
                            ...
                            <option value="usual" {{ old('preference_function', $criteria->preference_function) == 'usual'    ? 'selected' : '' }}>Usual
                            </option>
                            <option value="quasi"    {{ old('preference_function', $criteria->preference_function) == 'quasi'    ? 'selected' : '' }}>Quasi
                            </option>
                            <option value="linear"   {{ old('preference_function', $criteria->preference_function) == 'linear'   ? 'selected' : '' }}>Linear
                            </option>
                            <option value="level"    {{ old('preference_function', $criteria->preference_function) == 'level'    ? 'selected' : '' }}>Level
                            </option>
                            <option value="gaussian" {{ old('preference_function', $criteria->preference_function) == 'gaussian' ? 'selected' : '' }}>
                                Gaussian</option>
                            <option value="v_shape"  {{ old('preference_function', $criteria->preference_function) == 'v_shape'  ? 'selected' : '' }}>
                                V-Shape</option>
                        </select>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden mb-6"
                ...
                <div class="px-6 py-6 space-y-5">
                    ...
                    <div x-show="['quasi','linear','level'].includes(preferenceFunction)" x-transition x-cloak>
                        ...
                        <input type="number" id="param_q" name="param_q" value="{{ old('param_q',$criteria->param_q) }}"
                    ...
                    <div x-show="['linear','level','v_shape'].includes(preferenceFunction)" x-transition x-cloak>
                        ...
                        <input type="number" id="param_p" name="param_p" value="{{ old('param_p',     $criteria->param_p) }}"
                    ...
                    <div x-show="preferenceFunction === 'gaussian'" x-transition x-cloak>
                        ...
                        <input type="number" id="param_sigma" name="param_sigma" value="{{ old('param_sigma', $criteria->param_sigma) }}"
                    ...
                    <template x-for="(row, index) in rows" :key="index">
                        <div class="grid grid-cols-12 gap-3 items-center">
                            ...
                            <div class="col-span-2 flex items-center gap-2">
                                ...
                                <input type="hidden" :name="`ordinal[${index}][id]`" :value="row.id ?? ''">
                                {{-- Hapus baris --}}
