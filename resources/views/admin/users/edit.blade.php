...
@section('content')
    @php
        // Normalisasi: handle baik boolean maupun string
        $currentStatus = old('account_status') ?? ($user->account_status ? 'active' : 'inactive');
    @endphp
    <div class="mx-auto px-4 py-2">

        {{-- Page Header --}}
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('user.index') }}" aria-label="Kembali ke daftar user"
        ...
        <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            <form action="{{ route('user.update', $user->id) }}" method="POST" enctype="multipart/form-data" novalidate
                ...
                <fieldset class="p-6 border-b border-gray-100">
                    <legend
                        class="w-full text-sm font-semibold text-[var(--flora-moss)] py-5 pb-5 flex items-center gap-2.5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span
                                    class="absolute inset-y-0 left-3.5 flex items-center text-gray-400 text-sm pointer-events-none select-none"
                            ...
                            <p id="phone-hint" class="mt-1.5 text-xs text-gray-400">Opsional. Format: 08xx atau tanpa angka
                                0.</p>
                            @error('phone_number')
                ...
                <fieldset class="p-6 border-b border-gray-100">
                    <legend
                        class="w-full text-sm font-semibold text-[var(--flora-moss)] py-5 pb-5 flex items-center gap-2.5">
                        <i class="fa-solid fa-shield-halved text-[var(--flora-teal)] w-4 text-center"
                            aria-hidden="true"></i>
                        Hak Akses &amp; Status
                    </legend>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                        ...
                        {{-- Role --}}
                        <div>
                            <label for="roles_id" class="block text-sm font-medium text-gray-700 mb-1.5">
                                Role
                                <span class="text-red-500 ml-0.5" aria-hidden="true">*</span>
                                <span class="sr-only">(wajib diisi)</span>
                            </label>
                            <div class="relative">
                                <select id="roles_id" name="roles_id" required aria-required="true"
                                    ...
                                    {{-- <option value="{{ $user->roles_id }}" disabled {{ $user->roles_id ? '' : 'selected' }}>{{ $user->roles->name }}</option> --}}
                            ...
                            <p id="role-hint" class="mt-1.5 text-xs text-gray-400">Menentukan hak akses user dalam sistem.
                            </p>
                            @error('roles_id')
                        ...
                        <div>
                            ...
                            <div role="radiogroup" aria-labelledby="status-label"
                                ...
                                {{-- Active --}}
                                <label
                                    class="flex-1 flex items-center gap-3 px-4 py-3 border rounded-lg cursor-pointer transition-colors has-[:checked]:border-[var(--flora-teal)] has-[:checked]:bg-[var(--flora-teal)]/5 {{ $currentStatus === 'active' ? 'border-[var(--flora-teal)] bg-[var(--flora-teal)]/5' : 'border-gray-200 hover:border-gray-300' }}">
                                    <input type="radio" name="account_status" id="status_active"
                                        value="active"{{ $currentStatus === 'active' ? 'checked' : '' }} class="sr-only">
                                    <span
                                        class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0
                                        {{ $currentStatus === 'active' ? 'border-[var(--flora-teal)]' : 'border-gray-300' }}"
                                        aria-hidden="true">
                                        <span
                                            class="w-2 h-2 rounded-full {{ $currentStatus === 'active' ? 'bg-[var(--flora-teal)]' : '' }}"></span>
                                    </span>
                                    <div>
                                ...
                                {{-- Inactive --}}
                                <label
                                    class="flex-1 flex items-center gap-3 px-4 py-3 border rounded-lg cursor-pointer transition-colors has-[:checked]:border-[var(--flora-teal)] has-[:checked]:bg-[var(--flora-teal)]/5 {{ $currentStatus === 'inactive' ? 'border-[var(--flora-teal)] bg-[var(--flora-teal)]/5' : 'border-gray-200 hover:border-gray-300' }}">
                                    <input type="radio" name="account_status" id="status_inactive" value="inactive"
                                        {{ $currentStatus === 'inactive' ? 'checked' : '' }} class="sr-only">
                                    <span
                                        class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0
                                        {{ $currentStatus === 'inactive' ? 'border-[var(--flora-teal)]' : 'border-gray-300' }}"
                                        aria-hidden="true">
                                        <span
                                            class="w-2 h-2 rounded-full {{ $currentStatus === 'inactive' ? 'bg-[var(--flora-teal)]' : '' }}"></span>
                                    </span>
                                    <div>
                ...
                <fieldset class="p-6 border-b border-gray-100">
                    ...
                    {{-- Toggle checkbox --}}
                    <label for="is_collector" class="inline-flex items-start gap-3 mb-5 cursor-pointer group">
                        <div class="relative mt-0.5 shrink-0">
                            <input type="checkbox" id="is_collector" name="is_collector" value="1"
                                {{ old('is_collector', $user->collectorInfo ? true : false) ? 'checked' : '' }}
                                aria-describedby="collector-toggle-hint" aria-controls="collector-fields"
                                class="peer sr-only">
                            {{-- Custom checkbox --}}
                    ...
                    {{-- Collector Fields --}}
                    <div id="collector-fields" aria-live="polite"
                        class="{{ old('is_collector', $user->collectorInfo ? true : false) ? '' : 'hidden' }}">
                        ...
                        <div class="rounded-xl border border-[var(--flora-sage-mid)] bg-gray-50/60 p-5">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="collector_initial_name" name="collector_initial_name"
                                        value="{{ old('collector_initial_name', $user->collectorInfo?->initial_collector_name) }}"
                                        autocomplete="off"
                                        ...
                                        placeholder="Cth: JD" maxlength="10"
                                ...
                                <div>
                                    ...
                                    <input type="text" id="collector_display_name" name="collector_display_name"
                                        value="{{ old('collector_display_name', $user->collectorInfo?->full_name) }}"
                                        autocomplete="off"
                            ...
                            <div class="mt-5 flex items-start gap-2.5 px-4 py-3 bg-blue-50 border border-blue-100 rounded-lg"
                                role="note" aria-label="Informasi penyimpanan data collector">
                                <i class="fa-solid fa-circle-info text-blue-400 text-sm mt-0.5 shrink-0"
                                    aria-hidden="true"></i>
                                <p class="text-xs text-blue-700 leading-relaxed">
                                    Data pada bagian ini akan disimpan ke <strong>database collector</strong> secara
                                    terpisah dari data user utama.
        ...
        document.querySelectorAll('input[name="account_status"]').forEach(radio => {
            radio.addEventListener('change', () => {
                document.querySelectorAll('input[name="account_status"]').forEach(r => {
                    ...
                    dot.className =
                    `w-2 h-2 rounded-full ${active ? 'bg-[var(--flora-teal)]' : ''}`;
